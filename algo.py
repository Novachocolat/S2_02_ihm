...
from math import sqrt # Racine carrée pour les calculs mathématiques
...
def load_grid_from_json(json_path: str) -> Tuple[np.ndarray, Tuple[int, int], List[Tuple[int, int]]]:
    ...
    for cell in cells:
        ...
        # Obstacles
        if typ == "Rayon" or typ == "Mur" or typ == "Caisse":
            grid[row, col] = 1
        if typ == "Entrée":
            entry = (row, col)
        if typ == "Caisse":
...
# ==============================================================
# Algorithme A* (altération méliorative de l'algorithme de Dijkstra)
# Source : https://www.datacamp.com/fr/tutorial/a-star-algorithm?dc_referrer=https%3A%2F%2Fwww.google.com%2F
# ==============================================================
...
def find_shopping_points(cells, shopping_list, grid):
    """Retourne la liste des coordonnées à visiter pour chaque élément de la liste de courses."""
    points = []
    for item in shopping_list:
        for cell in cells:
            if cell.get("type") == "Rayon" and cell.get("object", "").lower() == item.lower():
                row, col = cell["row"], cell["col"]
                access = find_accessible_neighbor(grid, row, col)
                if access:
                    points.append(access)
                else:
                    print(f"⚠️  Aucun accès libre autour du rayon '{item}' en ({row},{col})")
                break
        else:
...
if __name__ == "__main__":
    json_path = "json/plan_test.json"
    shopping_list = ["Légumes", "Fruits", "Viandes", "Crèmerie", "Bureau"] # exemple de liste de course
